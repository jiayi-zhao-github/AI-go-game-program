...
def make_onehot(coords):
    ...
    for i, coord in enumerate(coords):
        output[i, p1.flatten_coords(coord)] = 1
    return output
...
class DataSet(object):
    def __init__(self, pos_features, next_moves, results, is_test=False):
        ...
        assert pos_features.shape[0] == next_moves.shape[0]
...
def parse_data_sets(*data_sets):
    ...
    print(" 找到%ssgfs." % len(sgf_files), file=sys.stderr)
